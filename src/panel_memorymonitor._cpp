...
void gui::PanelMemoryMonitor::_draw draw_function_stub
{
    ...
    Colour current_block_colour = Colour{ 255,64,64 };
    uint32_t pixel_index = 0;

    memory::MemoryFrame* current = (memory::MemoryFrame*)memory::memory_information.head;
    ...
    while (current->next != 0x0)
    {
        ...
        for (uint32_t i = 0; i < block_size_rounded; i++)
        {
            ...
            uint32_t real_pixel_index = graphics::getOffset(UVector2{ pixel_index % frame.size.u, pixel_index / frame.size.u } + frame.origin, framebuffer.size);
            graphics::setPixel(real_pixel_index, current->is_free ? nov_colour_nearblack : current_block_colour, framebuffer);
            pixel_index++;
        }
        // update current block colour
        current_block_colour = Colour{ (uint8_t)(current_block_colour.x + 64), (uint8_t)(current_block_colour.y + 64), (uint8_t)(current_block_colour.z + 32) };
